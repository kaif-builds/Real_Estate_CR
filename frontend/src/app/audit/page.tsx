...
function ActionBadge({ action }: { action: AuditLogRow['action'] }) {
  switch (action) {
    ...
    case 'Query':
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-cyan-50 text-cyan-700 border border-cyan-200">
          Query
        </span>
      )
    case 'Clarification':
      ...
      <div>
        ...
        <div className="ml-7">
          <div className="grid grid-cols-2 gap-1 bg-slate-50 border border-slate-200 rounded-lg p-2">
            {Object.entries(trail.proposed_values).filter(([, v]) => v != null && v !== '').length > 0 ? (
              ...
              <span className="text-xs text-slate-400 italic">No structured values proposed (query/clarification)</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="ml-7">
          <div className="grid grid-cols-2 gap-1 bg-emerald-50 border border-emerald-200 rounded-lg p-2">
            {Object.entries(trail.final_values).filter(([, v]) => v != null && v !== '').length > 0 ? (
              ...
              <span className="text-xs text-slate-400 italic">None (cancelled / query)</span>
            )}
          </div>
        </div>
      </div>
...
// ── Audit Page Content (inside Suspense for useSearchParams) ───────────────────
...
function AuditPageContent() {
  ...
  // Initialize/sync filters from URL params (drill-down support)
  useEffect(() => {
    if (!searchParams) return
    const ai = searchParams.get('ai')
    if (ai) setAiFilter(ai.toUpperCase())
  ...
  const filteredLogs = useMemo(() => {
    return MOCK_AUDIT_LOGS.filter((log) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesRequest = log.ai_trail?.original_request?.toLowerCase().includes(q)
        if (!matchesId && !matchesEntityId && !matchesUser && !matchesSummary && !matchesRequest) {
          return false
        }
      }
      return true
    })
  }, [entityFilter, userFilter, actionFilter, dateFrom, dateTo, searchQuery, aiFilter])
  ...
  const hasActiveFilters = entityFilter !== 'ALL' || userFilter !== 'ALL' || actionFilter !== 'ALL' || dateFrom || dateTo || searchQuery || aiFilter !== 'ALL'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link href="/ai-management">
            <Button variant="outline" size="sm" className="text-xs gap-1.5 border-amber-200 text-amber-800 hover:bg-amber-50">
        ...
        <CardContent className="p-4 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-7 gap-3">
            ...
            <div className="relative md:col-span-2">
              ...
              <Input
                placeholder="Search ID, summary, entity, request..."
                value={searchQuery}
            ...
            <div>
              ...
              >
                ...
                <option value="Query">Query (Read-only)</option>
          ...
          {hasActiveFilters && (
            <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs text-slate-600">
              <span className="flex items-center gap-1.5">
                ...
                {aiFilter === 'YES' && <Badge className="bg-amber-100 text-amber-800 border-amber-200 text-[10px] py-0">AI-assisted only</Badge>}
                {aiFilter === 'NO' && <Badge className="bg-slate-100 text-slate-700 border-slate-300 text-[10px] py-0">Manual only</Badge>}
                {actionFilter !== 'ALL' && <Badge variant="outline" className="text-[10px] py-0">Action: {actionFilter}</Badge>}
                {entityFilter !== 'ALL' && <Badge variant="outline" className="text-[10px] py-0">Entity: {entityFilter}</Badge>}
                {userFilter !== 'ALL' && <Badge variant="outline" className="text-[10px] py-0">User: {userFilter}</Badge>}
              </span>
              <button
...
export default function AuditPage() {
  return (
    <AppLayout>
      <Suspense fallback={<div className="p-6 text-sm text-slate-400">Loading audit log...</div>}>
        <AuditPageContent />
      </Suspense>
    </AppLayout>
  )
